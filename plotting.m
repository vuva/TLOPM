...
prefix='D:\Data\iperf-maxlag-loss0.01\';
...
for i=k:n
    lrtt_dat = csvread(strcat(prefix,exp_name,'-tag-1-',num2str(i), '.dat' ));
...
lrtt_latency = lrtt_latency(lrtt_latency(:, end) <.2, :);
rr_latency = rr_latency(rr_latency(:, end) >0.00, :);
rr_latency = rr_latency(rr_latency(:, end) <.2, :);
re_latency = re_latency(re_latency(:, end) >0.00, :);
re_latency = re_latency(re_latency(:, end) <.2, :);
sp_latency = sp_latency(sp_latency(:, end) >0.00, :);
sp_latency = sp_latency(sp_latency(:, end) <.2, :);
